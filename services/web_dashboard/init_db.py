...
# Create a new Flask app for initialization only
app = Flask(__name__)

# Ensure instance folder exists and configure SQLAlchemy with an absolute SQLite path
try:
    os.makedirs(app.instance_path, exist_ok=True)
except Exception:
    os.makedirs('instance', exist_ok=True)

sqlite_db_path = os.path.join(app.instance_path, 'healthcare.db')
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{sqlite_db_path}"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Import and initialize the shared database instance
...
def init_db():
    with app.app_context():
        ...
        if User.query.count() == 0:
            print("Initializing database with default users and sample data...")
